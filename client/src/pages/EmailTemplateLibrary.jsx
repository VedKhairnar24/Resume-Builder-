...
const templatesData = [
  {
    ...
    scenario: 'Job Search',
    tone: 'Formal',
    preview: 'I wanted to follow up regarding my application for the Software Engineer Intern role...',
  ...
  },
  {
    ...
    tone: 'Professional Casual',
    preview: 'My name is [Your Name], and I am a current student at [Your University]...',
  ...
  },
  {
    ...
    tone: 'Formal',
    preview: 'I am writing to request a letter of recommendation for my application to...',
  ...
  },
  {
    ...
    tone: 'Friendly',
    preview: 'Just a quick update on the [Project Name] project. We have completed...',
...
const EmailTemplateLibrary = () => {
  ...
  const filteredTemplates = templatesData.filter(template => {
    return activeScenario === 'All' || template.scenario === activeScenario;
  });
    ...
    <div className="min-h-screen bg-gray-50 py-10 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <header className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-3">Email & Communication Library</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white p-4 rounded-xl shadow-sm">
              <h3 className="text-sm font-semibold text-gray-500 uppercase mb-3">Scenarios</h3>
              <div className="flex flex-wrap gap-2">
                {scenarios.map(scenario => (
                  <button
                    key={scenario}
                    onClick={() => setActiveScenario(scenario)}
                    className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-200 ${activeScenario === scenario ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    {scenario}
                  </button>
                ))}
            ...
            <div className="space-y-3">
              {filteredTemplates.map(template => (
                <div
                  ...
                  className={`p-4 rounded-xl cursor-pointer transition-all duration-200 ${selectedTemplate.id === template.id ? 'bg-blue-100 border-2 border-blue-500 shadow-lg' : 'bg-white hover:bg-gray-50 border-2 border-transparent'}`}
                >
                  <div className="flex justify-between items-start">
                    <h4 className="font-bold text-gray-800">{template.title}</h4>
                    <button onClick={(e) => { e.stopPropagation(); toggleSave(template.id); }} title="Save Template">
                      <Star className={`w-5 h-5 ${saved.includes(template.id) ? 'text-yellow-500 fill-current' : 'text-gray-400 hover:text-yellow-400'}`} />
                    </button>
                  </div>
                  <p className="text-sm text-gray-600 mt-1 truncate">{template.preview}</p>
                  <span className="text-xs font-medium bg-gray-200 text-gray-700 px-2 py-0.5 rounded-full mt-2 inline-block">{template.tone}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Right Column: Editor and Tips */}
              ...
              <div className="bg-white p-6 rounded-xl shadow-sm">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-2xl font-semibold text-gray-800 flex items-center gap-2">
                    <Mail className="w-6 h-6 text-blue-500" /> {selectedTemplate.title}
                  </h2>
                  <button
                    onClick={handleCopy}
                ...
                <div className="bg-gray-50 p-4 rounded-lg border">
                  <p className="text-sm font-medium text-gray-800">Subject: {selectedTemplate.subject}</p>
                  <hr className="my-3" />
                  <textarea
                    value={selectedTemplate.fullText}
                    onChange={(e) => setSelectedTemplate(prev => ({ ...prev, fullText: e.target.value }))}
            ...
            {saved.length > 0 && (
              <div className="bg-white p-6 rounded-xl shadow-sm">
                ...
                <div className="flex flex-wrap gap-3">
                  {templatesData.filter(t => saved.includes(t.id)).map(template => (
                    <div key={template.id} onClick={() => handleSelectTemplate(template)} className="bg-yellow-50 border border-yellow-200 px-3 py-2 rounded-lg cursor-pointer hover:bg-yellow-100">
                      <p className="text-sm font-medium text-yellow-900">{template.title}</p>
                    </div>
                  ))}
                </div>
              </div>
