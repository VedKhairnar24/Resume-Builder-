...
const Register = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'firstName') {
      setFormData({ ...formData, name: { ...formData.name, first: value } });

    } else if (name === 'lastName') {
      setFormData({ ...formData, name: { ...formData.name, last: value } });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (passwordStrength < 4) {
      setError('Password is not strong enough. Please improve your password strength.');
      return;
    }
  ...
  const handleGoogleLogin = () => {
    alert("You will be redirected to Google for authentication");
